...
(() => {
  ...
  /* ---------- totaux par groupe (une ligne par lot de cartes échangées) ---------- */
  WMP.renderTotals = (items, getStats) => {
    const groups = new Map();
    for (const it of items) {
      if (!it.group) continue;
      let arr = groups.get(it.group);
      if (!arr) groups.set(it.group, (arr = []));
      arr.push(it);
    }
    for (const [host, arr] of groups) {
      ...
      if (!WMP.settings.badges || !arr.length) {
        if (node) node.remove();
        continue;
      }
      if (!node) {
      ...
      node.textContent =
        `Total ◈ ${WMP.num(sum)}` + (missing ? ` · ${missing} sans prix` : "");
      node.title = `Somme des derniers prix de ${priced} carte(s) sur ${arr.length}`;
    }
  };
